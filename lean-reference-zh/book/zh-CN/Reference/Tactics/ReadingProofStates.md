...
n d : Nat
hd : n + d = d + n
⊢ n + Nat.succ d = Nat.succ d + n
```

解读：

- `case succ` — 当前在归纳的 `succ` 分支（具名 case）；
- `n d : Nat` — 类型为 `Nat` 的变量；
- `hd : n + d = d + n` — 命题假设（归纳假设）；
- `⊢ ...` — 待证结论。

不可访问假设可能显示为 `x† : T`，不能直接写 `exact x`，但 `assumption` 仍可用。

## InfoView 与终端

- **InfoView**（光标放在 tactic 行）：实时看 main goal，不必 `lake build`；
- **`trace_state`**：把 goal 打到构建日志，适合 CI 或没有 InfoView 时；
- 二者显示格式相同，都遵循上述 assumption + `⊢` 结构。
