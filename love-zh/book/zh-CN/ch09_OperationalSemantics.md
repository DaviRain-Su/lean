...
> 我们完成了 WebAssembly 语言核心执行语义与类型系统的完整 Isabelle 机械化。为了完成类型可靠性证明，我们的证明与建模工作揭露了官方 WebAssembly 规范中的**若干缺陷**，需要由规范作者予以修正。在某些情况下，这意味着类型系统**原本是不健全的**。
>
...
形式语义有助于规定并推理编程语言本身，以及各个程序。它可以成为已验证编译器、解释器、验证器、静态分析器、类型检查器等的基础。若没有形式化证明，这些工具**几乎总是错误的**。

在这一领域，证明助手被广泛使用。每年约有 10–20% 的 POPL 论文得到部分或完全形式化。成功的原因包括：

* 入门所需机制（背景库、策略）较少，超出归纳类型、归纳谓词与递归函数之外的需求不多；
* 证明往往包含大量分情况讨论，非常适合计算机；
* 证明助手能在我们扩展编程语言、增添更多特性时，跟踪需要修改的内容。
...
**操作语义**（operational semantics）对应于理想化的解释器（以 Prolog 风格语言规定）。主要有两种变体：**大步语义**（big-step semantics）与**小步语义**（small-step semantics）。我们先为 WHILE 语言给出大步语义。
...
与递归函数相比，使用归纳谓词可以处理非终止（发散的 `while`），以及对 WHILE 更丰富的语言中的非确定性。它 arguably 也提供了更贴近科学文献中传统判断规则的语法。若我们尝试如下递归定义：
...
* 证明编程语言的性质，例如程序之间的**等价性证明**与**确定性**（determinism）；
* 对**具体程序**进行推理，证明将终态 `t` 与初态 `s` 联系起来的定理。

大步语义使我们能够对具体程序进行推理，证明将终态与初态联系起来的定理。同样重要的是，它使我们能够证明编程语言的性质，例如确定性与非终止性。
...
出于技术原因，对 `(S, s)` 用单一变量 `Ss` 表示。证明在演示文件中给出。非形式证明梗概如下：
