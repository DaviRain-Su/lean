...
其中 `aₙ` 是满足 `aₙ² < 2` 的、小数点后恰好有 `n` 位数字的最大数。该序列似乎收敛，因为每个 `aₙ` 与后续任意一项的距离至多为 `10⁻ⁿ`，但其极限是 `√2 ∉ ℚ`。从这个意义上说，有理数是不**完备**的，而实数正是它们的**完备化**（completion）。要构造实数，我们需要填补这些「看似收敛却不收敛」的序列所揭示的缺口。
